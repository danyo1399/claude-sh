...
# -------------------------------------------------------------------
# cxralph.sh
# RALPH (Recursive Agentic Loop Pipeline Harness) — Codex edition:
#   Repeatedly invokes Codex with a user prompt until the AI
...
usage() {
  cat <<EOF
RALPH — Recursive Agentic Loop Pipeline Harness (Codex).

Repeatedly invokes Codex with a user prompt in a loop. Each iteration
is a fresh Codex session. The loop exits when Codex includes the
marker text RALPH_DONE_<SUFFIX> in its response, indicating it found
nothing left to do.

Usage: cxralph.sh [OPTIONS] -- PROMPT
       cxralph.sh [OPTIONS] --prompt-file FILE

Arguments:
  PROMPT               The prompt to send to Codex on each iteration.
...
Examples:
  cxralph.sh -- "Review all TODO comments in this repo and fix them one at a time"
  cxralph.sh --model gpt-5-codex -- "Find and fix lint warnings in src/"
  cxralph.sh --max-iterations 10 -- "Refactor functions longer than 50 lines"
  cxralph.sh --prompt-file my-task.md
EOF
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'cxralph.sh --help' for usage." >&2
      exit 1
      ;;
    *)
      USER_PROMPT="$*"
...
WORK_DIR="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR%/}/cxralph-${RUN_ID}"
mkdir -p "$WORK_DIR"
...
# -- run_codex helper ────────────────────────────────────────────
# run_codex PROMPT STDOUT_FILE [--continue]
#   Runs codex exec (or codex exec resume --last to resume last session),
...
run_codex() {
  ...
  if [ "$continue_flag" = "--continue" ]; then
    cmd_args=(exec resume --last "$prompt")
  else
    cmd_args=(exec "$prompt")
  fi

  "$CODEX_CMD" "${cmd_args[@]}" ${model_args[@]+"${model_args[@]}"} \
...
while [ "$ITERATION" -lt "$MAX_ITERATIONS" ]; do
  ...
  if [ "$CONSECUTIVE_CONTINUES" -gt 0 ]; then
    ...
    if ! run_codex "continue" "$ITER_OUTPUT" --continue; then
      echo "ERROR: Iteration ${ITERATION} failed — codex exited with a non-zero status." >&2
      echo "  See log: ${LOG_FILE}" >&2
      exit 1
    fi
  else
    # Fresh session with full prompt
    if ! run_codex "$FULL_PROMPT" "$ITER_OUTPUT"; then
      echo "ERROR: Iteration ${ITERATION} failed — codex exited with a non-zero status." >&2
      echo "  See log: ${LOG_FILE}" >&2
      exit 1
    fi
  ...
  if [ ! -s "$ITER_OUTPUT" ]; then
    ...
    echo "  Codex ran but did not produce output." >&2
    echo "  See log: ${LOG_FILE}" >&2
    exit 1
  fi
