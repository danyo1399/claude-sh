...
usage() {
  cat <<EOF
AI code review pipeline that drafts a review of your branch changes,
audits the draft to produce a validated final review, and optionally
auto-fixes the issues found.

Usage: cxcodereview.sh [OPTIONS] [BASE_BRANCH]
...
Examples:
  cxcodereview.sh                        # review branch vs main
  cxcodereview.sh develop                # review branch vs develop
  cxcodereview.sh --staged               # review staged changes only
  cxcodereview.sh --pending              # review all uncommitted changes
  cxcodereview.sh --fix                  # review and auto-fix issues
  cxcodereview.sh --model gpt-5-codex feature/main
EOF
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'cxcodereview.sh --help' for usage." >&2
      exit 1
      ;;
    *)
      BASE_BRANCH="$1"
...
WORK_DIR="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR%/}/cx-code-review-${RUN_ID}"
mkdir -p "$WORK_DIR"
...
echo "── Code Review Pipeline (Codex) ─────────────────────────"
...
# ══════════════════════════════════════════════════════════════════
echo "── Step 1/${TOTAL_STEPS}: Draft Code Review ──────────────────────────"
echo ""
...
if ! run_codex "$PROMPT_DRAFT"; then
  echo "ERROR: Step 1 failed — codex exited with a non-zero status." >&2
...
# ══════════════════════════════════════════════════════════════════
echo "── Step 2/${TOTAL_STEPS}: Audit & Finalise ───────────────────────────"
echo ""
...
if ! run_codex "$PROMPT_FINAL" "$REVIEW_OUTPUT"; then
  echo "ERROR: Step 2 failed — codex exited with a non-zero status." >&2
  echo "  See log: ${LOG_FILE}" >&2
  exit 1
fi
...
# ══════════════════════════════════════════════════════════════════
if [ "$AUTO_FIX" = true ]; then
  echo "── Step 3/${TOTAL_STEPS}: Auto-fix ────────────────────────────────"
  echo ""

  PROMPT_CONTEXT="CONTEXT:
- Repository root: ${REPO_ROOT}
- Current branch: ${CURRENT_BRANCH}
- Code review file: ${REVIEW_OUTPUT}"

  PROMPT_BODY=$(cat <<'PROMPT_EOF'

You are a senior software engineer. Your job is to fix all valid issues identified in a code review.

INSTRUCTIONS:

Read the code review from the code review file listed above. For each issue in the review:

1. Read the relevant source file(s) and understand the surrounding code
2. Apply a fix that resolves the issue while preserving existing behaviour
3. Follow the existing code style and conventions of each file

## Rules

- Fix ONLY the issues listed in the code review — do not refactor, clean up, or improve other code
- Do not add comments explaining your fixes
- Do not change code formatting or whitespace beyond what is needed for the fix
- If an issue cannot be safely fixed without more context or would require a design change, skip it and list it at the end as unfixed
- Verify each fix does not break surrounding logic before moving on
- After all fixes are applied, print a summary listing each issue and whether it was fixed or skipped (with reason)
PROMPT_EOF
  )
...
${PROMPT_BODY}"
  ...
  if ! run_codex "$PROMPT_FIX"; then
    echo "ERROR: Step 3 failed — codex exited with a non-zero status." >&2
    echo "  See log: ${LOG_FILE}" >&2
    exit 1
  fi
  echo ""
