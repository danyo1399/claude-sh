...
# -------------------------------------------------------------------
# cxralphtask.sh
# RALPH Task Pipeline (Codex edition):
...
#                 Codex what the next task should be (fresh session),
...
#   3. (builder)  Fresh Codex session: pick up the task (in-progress
...
usage() {
  cat <<EOF
RALPH Task Pipeline (Codex) — planner/builder loop with file-system task tracking.
...
Usage: cxralphtask.sh [OPTIONS] --goal-file FILE --template-file FILE --guidelines-file FILE
...
Environment variables:
  CODEX_CMD                 Path to codex binary; --codex-cmd takes precedence
  CODEX_MODEL               Model override; --model takes precedence
...
Examples:
  ...
  cxralphtask.sh --model gpt-5-codex --tasks-dir ./project-tasks \\
    --max-iterations 100 \\
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'cxralphtask.sh --help' for usage." >&2
      exit 1
      ;;
    *)
...
WORK_DIR="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR%/}/cxralphtask-${RUN_ID}"
mkdir -p "$WORK_DIR"
...
run_codex() {
  ...
  if [ "$DEBUG" = "1" ]; then
    {
      echo ""
      echo "── prompt sent to codex (continue=${continue_flag:-no}) ──"
      printf '%s\n' "$prompt"
      echo "── end prompt ──"
      echo ""
    } | tee -a "$LOG_FILE" >&2
...
# -- Banner ──────────────────────────────────────────────────────
echo "── RALPH Task Pipeline (Codex) ───────────────────────────────"
if [ -n "$REPO_ROOT" ]; then
...
Otherwise, end with a brief description of the next task (no marker)."
    ...
    if ! run_codex "$PLANNER_PROMPT" "$PLAN_OUTPUT"; then
...
confirm the absolute path of the file you created."
    ...
    if ! run_codex "$CREATE_PROMPT" "$CREATE_OUTPUT" --continue; then
...
verified. ${TASKS_DIR}/in-progress/ should be empty when you finish."
  ...
  while [ "$ATTEMPT" -le "$MAX_BUILDER_ATTEMPTS" ]; do
    if [ "$ATTEMPT" = "1" ]; then
      ...
      if run_codex "$BUILDER_PROMPT" "$BUILD_OUTPUT"; then
        BUILDER_OK=1
        break
      fi
    else
      echo "  Builder crashed; retrying with --continue (attempt ${ATTEMPT}/${MAX_BUILDER_ATTEMPTS})"
      if run_codex "continue" "$BUILD_OUTPUT" --continue; then
        BUILDER_OK=1
        break
      fi
    fi
...
if [ "$DRY_RUN" = "1" ]; then
  echo "── RALPH Task Pipeline Dry Run Complete (Codex) ──────────────"
else
  echo "── RALPH Task Pipeline Complete (Codex) ──────────────────────"
fi
