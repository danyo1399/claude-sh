...
# -------------------------------------------------------------------
# cxgitcommit.sh
# AI-powered git commit pipeline (Codex):
...
Environment variables:
  CODEX_CMD              Path to codex binary (default: codex)
...
Examples:
  cxgitcommit.sh                              # commit staged changes
  cxgitcommit.sh --all                        # stage and commit everything
  cxgitcommit.sh --smart                      # AI creates logical commits
  cxgitcommit.sh --staged --push              # commit staged and push
  cxgitcommit.sh --all --context "refactored auth module"
  cxgitcommit.sh --smart --push --context "feature: user dashboard"
EOF
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'cxgitcommit.sh --help' for usage." >&2
      exit 1
      ;;
    *)
...
WORK_DIR="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR%/}/cxgitcommit-${RUN_ID}"
mkdir -p "$WORK_DIR"
...
echo "── Git Commit Pipeline (Codex) ──────────────────────────"
...
${PROMPT_BODY}"
  ...
  if ! run_codex "$FULL_PROMPT"; then
    echo "ERROR: Smart commit failed — codex exited with a non-zero status." >&2
...
${PROMPT_BODY}"
  ...
  if ! run_codex "$FULL_PROMPT"; then
    echo "ERROR: Commit message generation failed — codex exited with a non-zero status." >&2
    echo "  See log: ${LOG_FILE}" >&2
    exit 1
  fi
