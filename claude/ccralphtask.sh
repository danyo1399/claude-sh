...
#                 Claude what the next task should be (fresh session),
...
#   3. (builder)  Fresh Claude session: pick up the task (in-progress
...
usage() {
  cat <<EOF
RALPH Task Pipeline (Claude) — planner/builder loop with file-system task tracking.
...
Usage: ccralphtask.sh [OPTIONS] --goal-file FILE --template-file FILE --guidelines-file FILE
...
Environment variables:
  CLAUDE_CMD                Path to claude binary; --claude-cmd takes precedence
  CLAUDE_MODEL              Model override; --model takes precedence
...
Examples:
  ...
  ccralphtask.sh --model claude-opus-4-6 --tasks-dir ./project-tasks \\
    --max-iterations 100 \\
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'ccralphtask.sh --help' for usage." >&2
      exit 1
      ;;
    *)
...
WORK_DIR="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR%/}/ccralphtask-${RUN_ID}"
mkdir -p "$WORK_DIR"
...
run_claude() {
  ...
  if [ "$DEBUG" = "1" ]; then
    {
      echo ""
      echo "── prompt sent to claude (continue=${continue_flag:-no}) ──"
      printf '%s\n' "$prompt"
      echo "── end prompt ──"
      echo ""
    } | tee -a "$LOG_FILE" >&2
...
# -- Banner ──────────────────────────────────────────────────────
echo "── RALPH Task Pipeline (Claude) ──────────────────────────────"
if [ -n "$REPO_ROOT" ]; then
...
Otherwise, end with a brief description of the next task (no marker)."
    ...
    if ! run_claude "$PLANNER_PROMPT" "$PLAN_OUTPUT"; then
...
confirm the absolute path of the file you created."
    ...
    if ! run_claude "$CREATE_PROMPT" "$CREATE_OUTPUT" --continue; then
...
verified. ${TASKS_DIR}/in-progress/ should be empty when you finish."
  ...
  while [ "$ATTEMPT" -le "$MAX_BUILDER_ATTEMPTS" ]; do
    if [ "$ATTEMPT" = "1" ]; then
      ...
      if run_claude "$BUILDER_PROMPT" "$BUILD_OUTPUT"; then
        BUILDER_OK=1
        break
      fi
    else
      echo "  Builder crashed; retrying with --continue (attempt ${ATTEMPT}/${MAX_BUILDER_ATTEMPTS})"
      if run_claude "continue" "$BUILD_OUTPUT" --continue; then
        BUILDER_OK=1
        break
      fi
    fi
...
if [ "$DRY_RUN" = "1" ]; then
  echo "── RALPH Task Pipeline Dry Run Complete (Claude) ─────────────"
else
  echo "── RALPH Task Pipeline Complete (Claude) ─────────────────────"
fi
