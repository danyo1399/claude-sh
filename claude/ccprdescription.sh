...
# -------------------------------------------------------------------
# ccprdescription.sh
# AI pull request description pipeline:
...
Usage: ccprdescription.sh [OPTIONS] [BASE_BRANCH]
...
Examples:
  ccprdescription.sh                        # compare current branch vs main
  ccprdescription.sh develop                # compare current branch vs develop
  ccprdescription.sh --model claude-sonnet-4-20250514 main
EOF
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)
      ...
      echo "  Run 'ccprdescription.sh --help' for usage." >&2
      exit 1
      ;;
    *)
      BASE_BRANCH="$1"
...
if ! run_claude "$FULL_PROMPT"; then
  echo "ERROR: PR description generation failed — claude exited with a non-zero status." >&2
